...
_STOP = {"the","and","or","of","for","to","a","an","in","on","at","by","with","from","llc","inc","co","corp","corporation","company","ltd","pllc","pc","dba","llp","mr","mrs","ms","dr",}
_ALLOW_SHORT = {"sa","tx","us","24x7","24/7"}
_SYNONYM_PAIRS = {"hvac":{"heating","cooling","air","conditioning","a/c"},"a/c":{"air","conditioning","ac"},"landscape":{"landscaping","lawn","yard"},"security":{"camera","cameras","surveillance","cctv"}}
_nonword = re.compile(r"[^a-z0-9]+"); _ws = re.compile(r"\s+")
def _tok(s: str)->list[str]:
    s=(s or "").lower().replace("&"," and "); s=_nonword.sub(" ",s); s=_ws.sub(" ",s).strip()
    if not s: return []
    out=[]; 
    for t in s.split(" "):
        if not t: continue
        if len(t)<=2 and t not in _ALLOW_SHORT: continue
...
def _email_tokens(email:str)->list[str]:
    ...
    local,dom=email.split("@",1); return _tok(local)+_tok(dom.replace("."," "))
def _website_tokens(url:str)->list[str]:
    url=(url or "").strip().lower()
    if not url: return []
    try:
        from urllib.parse import urlparse as _u; host=_u(url if "://" in url else "https://"+url).netloc
    except Exception: return []
    host=host.split(":")[0]; bits=[b for b in host.split(".") if b not in {"www","com","net","org","io","co","us"}]; return _tok(" ".join(bits))
@lru_cache(maxsize=1)
def _synonyms_from_db(conn)->dict[str,set[str]]:
    ...
    try:
        ...
        for base,alts in rows:
            ...
            import re as _re
            for a in _re.split(r"[,\s]+", alts or ""):
                a=a.strip().lower()
                if a and a!=base: alt_set.add(a)
    except Exception: pass
    symmetric={}
    for base,alts in syn.items():
        for a in alts: symmetric.setdefault(a,set()).add(base)
    for k,v in symmetric.items(): syn.setdefault(k,set()).update(v)
    return syn
def _seed_terms_for(conn,category:str,service:str)->set[str]:
    terms=set()
    try:
        q=("SELECT terms FROM ckw_seeds WHERE (category = ? AND service = ?) "
           "OR (category = ? AND service = '*') OR (category='*' AND service='*')")
        rows=conn.execute(q,(category or "", service or "", category or "")).fetchall()
        for (t,) in rows:
            for w in _tok(t or ""): terms.add(w)
    except Exception: pass
    return terms
def compute_ckw_row(row:dict,conn=None)->tuple[str,str]:
    bag=set()
    bag.update(_tok(row.get("business_name",""))); bag.update(_tok(row.get("category",""))); bag.update(_tok(row.get("service",""))); bag.update(_tok(row.get("contact_name",""))); bag.update(_tok(row.get("notes",""))); bag.update(_tok(row.get("keywords","")))
    bag.update(_email_tokens(row.get("email",""))); bag.update(_website_tokens(row.get("website","")))
    if conn is not None: bag.update(_seed_terms_for(conn,row.get("category",""),row.get("service","")))
    bag.update(_tok(row.get("ckw_manual_extra","")))
    if conn is not None:
        syn=_synonyms_from_db(conn); expand=set()
        for t in list(bag): expand.update(syn.get(t,()))
        bag.update(expand)
    bag={t for t in bag if t not in _STOP}
    ordered=sorted(bag,key=lambda t:(-len(t),t))
    return " ".join(ordered), CURRENT_CKW_VER
def recompute_ckw_for_df(df,conn=None):
    if "ckw_locked" not in df.columns: df["ckw_locked"]=0
    if "ckw_version" not in df.columns: df["ckw_version"]=""
    changed=0; out_ckw=[]; out_ver=[]
    for _,r in df.iterrows():
        if int(r.get("ckw_locked",0) or 0)==1:
            cur=str(r.get("ckw") or "")
            if r.get("ckw_manual_extra"):
                extras=_tok(r.get("ckw_manual_extra")); bag=set(cur.split())|set(extras)
                cur2=" ".join(sorted(bag,key=lambda t:(-len(t),t)))
                if cur2!=cur: changed+=1; out_ckw.append(cur2); out_ver.append(CURRENT_CKW_VER)
                else: out_ckw.append(cur); out_ver.append(r.get("ckw_version") or CURRENT_CKW_VER)
            else: out_ckw.append(cur); out_ver.append(r.get("ckw_version") or CURRENT_CKW_VER)
            continue
        ckw_str,ver=compute_ckw_row(r.to_dict(),conn=conn)
        if ckw_str!=(r.get("ckw") or "") or ver!=(r.get("ckw_version") or ""): changed+=1
        out_ckw.append(ckw_str); out_ver.append(ver)
    df=df.copy(); df["ckw"]=out_ckw; df["ckw_version"]=out_ver; return df,changed
...
# 1) Insert engine after the last import if missing
if "=== ANCHOR: CKW ENGINE (start) ===" not in t:
    lines = t.splitlines()
    last_imp = -1
    for i, L in enumerate(lines[:200]):  # assume imports near top
        if L.startswith("import ") or L.startswith("from "):
            last_imp = i
    insert_at = last_imp + 1 if last_imp >= 0 else 0
    lines[insert_at:insert_at] = ["", engine_block]
    t = "\n".join(lines)

# 2) Append hook near end if missing
if "=== ANCHOR: CKW RECOMPUTE HOOK (start) ===" not in t:
    t = t.rstrip() + "\n\n" + hook_block + "\n"

Path("app_admin.py").write_text(t, encoding="utf-8")
print("Inserted CKW engine + hook anchors.")
PY
