...
_ORIG_AgGrid = globals().get("AgGrid")


def AgGrid(*args, **kwargs):
    st = globals().get("st")
    try:
        if callable(_ORIG_AgGrid):
            return _ORIG_AgGrid(*args, **kwargs)
    except Exception as ex:
        if st:
            st.warning(f"AgGrid failed; falling back to dataframe: {ex!r}")
    # Fallback: try to detect the dataframe argument
    df_arg = None
    if args:
        df_arg = args[0]
    else:
        # common kw names people pass
        for k in ("data", "df", "value"):
            if k in kwargs:
                df_arg = kwargs[k]
                break
    if st and df_arg is not None:
        try:
            st.dataframe(df_arg)
        except Exception as ex2:
            st.error(f"Fallback dataframe render failed: {ex2!r}")
    return None


# --- ANCHOR: ADMIN BROWSE — AgGrid safe shim (end) ---
# App constants
PHONE_LEN = 10
...
def _normalize_browse_df(
    ...
    # --- ANCHOR: ADMIN BROWSE - sanitize columns (start) ---
    view_cols = [c for c in view_cols if c in df.columns]
    if not view_cols:
        view_cols = [c for c in df.columns if c not in hidden_cols][: min(10, len(df.columns))]
    if isinstance(hidden_cols, set):
        hidden_cols = {c for c in hidden_cols if c in df.columns and c not in set(view_cols)}
    else:
        hidden_cols = [c for c in hidden_cols if c in df.columns and c not in set(view_cols)]
    # --- ANCHOR: ADMIN BROWSE - sanitize columns (end) ---
...
def _load_browse_help_md() -> str:
    ...
    # --- ANCHOR: ADMIN BROWSE — render (start) ---
    # Hidden set seeded from secrets (if any)
    try:
        _hide = st.secrets.get("HIDE_COLUMNS", [])
        hidden_cols = set(c for c in _hide if isinstance(c, str))
    except Exception:
        hidden_cols = set()

    # Ensure df exists (late-load if needed)
    if 'df' not in locals():
        try:
            eng = _engine()
            df = pd.read_sql("SELECT * FROM vendors", eng)
        except Exception as e:
            st.error(f"Browse load failed (late): {e!r}")
            return

    # Normalize df (order/phone/hidden/seed)
    try:
        df, view_cols, hidden_cols = _normalize_browse_df(df, hidden_cols)
    except Exception as ex:
        st.error(f"Browse normalize failed: {ex!r}")
        return

    # Build a safe view dataframe
    try:
        _df_view = df[view_cols] if view_cols else df
    except Exception:
        _df_view = df

    # Try AgGrid via shim; it will fallback to dataframe if AgGrid errors
    try:
        AgGrid(_df_view)
    except Exception as ex:
        st.warning(f"AgGrid exception; falling back to dataframe: {ex!r}")
        st.dataframe(_df_view)
    # --- ANCHOR: ADMIN BROWSE — render (end) ---
    return content
