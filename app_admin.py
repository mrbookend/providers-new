...
def main() -> None:
    ...
    with tab_browse:
        ...
        with st.expander("Browse diagnostics (click to open)", expanded=False):
            if _issues:
                ...
                st.caption(
                    "Shown: first 5 examples per column. Values are normalized for safe rendering/export."
                )
            else:
                st.caption(
                    "No obvious mixed types or hidden characters detected in the first 300 rows."
                )
    ...
    with tab_manage:
        if not st.session_state.get("DB_READY"):
            st.info(
                "Database not ready — skipping Add/Edit UI because required tables are missing."
            )
        else:
            eng = get_engine()
            ...
            with lc:
                ...
                cat_choice = ccol1.selectbox(
                    "Category *", options=["— Select —"] + cats, key="cat_add_sel"
                )
                cat_new = ccol2.text_input("New Category (optional)", key="cat_add_new")
                category = (cat_new or "").strip() or (
                    cat_choice if cat_choice != "— Select —" else ""
                )

                scol1, scol2 = st.columns([1, 1])
                srv_choice = scol1.selectbox(
                    "Service *", options=["— Select —"] + srvs, key="srv_add_sel"
                )
                srv_new = scol2.text_input("New Service (optional)", key="srv_add_new")
                service = (srv_new or "").strip() or (
                    srv_choice if srv_choice != "— Select —" else ""
                )
                ...
                if st.button(
                    "Add Provider", type="primary", disabled=disabled, key="btn_add_provider"
                ):
                    data = {
            ...
            with rc:
                ...
                if not rows:
                    ...
                    if r:
                        bn_e = st.text_input(
                            "Business Name *", value=r["business_name"], key="bn_edit"
                        )
            ...
            if not labels:
                st.warning(
                    "No providers to delete yet (missing table or empty list). Initialize or seed the database in **Maintenance**."
                )
                st.stop()
                    ...
                    try:
                        with eng.begin() as cx3:
                            ...
                            if hasattr(res, "rowcount") and res.rowcount == 0:
                                st.warning(
                                    f"No provider found with id={row['id']}. It may have been removed already."
                                )
                            else:
                                st.success(
                                    f"Deleted provider id={row['id']} ({row['business_name']})."
                                )
    ...
    with tab_catsvc:
        ...
        with cc:
            ...
            if del_cat != "— Select —":
                try:
                    ...
                    if cnt == 0 and st.button(
                        "Delete Category", type="secondary", key="btn_del_cat"
                    ):
                        with eng.begin() as cx:
                            cx.exec_driver_sql(
                                "DELETE FROM categories WHERE name=:n", {"n": del_cat}
                            )
                        st.session_state["DATA_VER"] += 1
            ...
            if cats:
                ...
                if (
                    st.button("Reassign Category for All Matching Vendors", key="btn_reassign_cat")
                    and from_cat
                    and to_cat.strip()
                ):
                    try:
                        ...
                        with eng.begin() as cx:
                            cx.exec_driver_sql(
                                "DELETE FROM categories WHERE name=:n", {"n": from_cat}
                            )
        ...
        with ss:
            ...
            if del_srv != "— Select —":
                try:
                    ...
                    st.caption(f"In use by {cnt} provider(s)")
                    if cnt == 0 and st.button(
                        "Delete Service", type="secondary", key="btn_del_srv"
                    ):
                        with eng.begin() as cx:
            ...
            if srvs:
                ...
                if (
                    st.button("Reassign Service for All Matching Vendors", key="btn_reassign_srv")
                    and from_srv
                    and to_srv.strip()
                ):
                    try:
                        ...
                        with eng.begin() as cx:
                            cx.exec_driver_sql(
                                "DELETE FROM services WHERE name=:n", {"n": from_srv}
                            )
                        changed = recompute_ckw_all(eng)
                        st.session_state["DATA_VER"] += 1
                        st.success(
    ...
    with tab_maint:
        ...
        st.info(
            "Add recompute/seed/repair tools here. (Existing Maintenance controls can be ported in.)"
        )
