...
def _auto_app_ver() -> str:
    ...
    short = _os.environ.get("GITHUB_SHA", "")
    if short:
        short = short[:7]
    else:
        try:
...
def _debug_where_am_i():
    """Show runtime file path, md5, mtime, CWD, and git branch/commit (best-effort)."""
    try:
        path = __file__
    except NameError:
        path = "(no __file__)"
...
# --- column widths helpers (st.dataframe config) ---
...
def _has_ckw_column(eng) -> bool:
    try:
        ...
        return any(str(r[1]) == "computed_keywords" for r in rows)
    except Exception:
        return False
    ...
    try:
        with eng.begin() as cx:
            ...
            if "ckw_version" not in cols:
                cx.exec_driver_sql("ALTER TABLE vendors ADD COLUMN ckw_version TEXT DEFAULT ''")
                changed = True
            if "ckw_locked" not in cols:
                cx.exec_driver_sql("ALTER TABLE vendors ADD COLUMN ckw_locked INTEGER DEFAULT 0")
                changed = True
            if "ckw_manual_extra" not in cols:
                cx.exec_driver_sql("ALTER TABLE vendors ADD COLUMN ckw_manual_extra TEXT DEFAULT ''")
                changed = True
            # index
            cx.exec_driver_sql(
...
DISABLE_LOGIN = _as_bool(_get_secret("DISABLE_ADMIN_PASSWORD", str(DISABLE_ADMIN_PASSWORD_DEFAULT)), True)
ADMIN_PASSWORD = (_get_secret("ADMIN_PASSWORD", ADMIN_PASSWORD_DEFAULT) or "").strip()
...
if not DISABLE_LOGIN:
    ...
    if not st.session_state["auth_ok"]:
        ...
        if st.button("Sign in"):
            if hmac.compare_digest((pw or "").strip(), ADMIN_PASSWORD):
...
def build_engine() -> Tuple[Engine, Dict]:
    ...
    embedded_path = os.path.abspath(_get_secret("EMBEDDED_DB_PATH", "vendors-embedded.db") or "vendors-embedded.db")

    if not url:
        db_path = _get_secret("DB_PATH", "vendors.db") or "vendors.db"
        eng = create_engine(
            f"sqlite:///{db_path}",
            pool_pre_ping=True,
            pool_recycle=300,
...
def ensure_schema(engine: Engine) -> None:
    """
    Create baseline schema INCLUDING CKW and phone_fmt/email so inserts never fail.
    """
    stmts = [
        """
...
def _seed_if_empty(eng=None) -> None:
    ...
    try:
        ...
        # Require minimal columns; others optional
        required = {"business_name", "category"}
        optional = {
            "service","phone","contact_name","address","website","email","notes",
            "keywords","computed_keywords","ckw_version","ckw_locked","ckw_manual_extra",
            "city","state","zip"
        }
        ...
        # phone digits-only; create phone_fmt display
        def _digits_only(x):
            s = re.sub(r"\D+", "", str(x or ""))
            return s

        df["phone"] = df["phone"].apply(_digits_only)
        def _fmt10(s: str) -> str:
            return f"({s[0:3]}) {s[3:6]}-{s[6:10]}" if len(s) == 10 else s
        df["phone_fmt"] = df["phone"].apply(_fmt10)

        # if no computed_keywords provided, leave blank (can recompute later)
        with eng.begin() as cx:
...
if submit:
    ...
    phone_fmt = f"({phone_digits[0:3]}) {phone_digits[3:6]}-{phone_digits[6:10]}" if len(phone_digits) == 10 else phone_raw
    ...
    try:
        ...
        ckw, ver = (" ".join(p for p in parts if p).strip(), CURRENT_CKW_VER)
...
def _prepare_csv_for_append(
    ...
    *,
    ...
    if "id" in csv_df.columns:
        dup_ids = (
            csv_df["id"].pipe(pd.to_numeric, errors="coerce").dropna().astype(int).duplicated(keep=False)
        )
        if dup_ids.any():
...
def _execute_append_only(
    ...
    with engine.begin() as conn:
        ...
        if not without_id_df.empty:
            cols = list(without_id_df.columns)
            placeholders = ", ".join(":" + c for c in cols)
            stmt = sql_text(f"INSERT INTO vendors ({', '.join(cols)}) VALUES ({placeholders})")
...
# ---- PAGE_SIZE from secrets (bounded) ---------------------------------------
...
# ---- Case-insensitive simple filter (CKW-first) -----------------------------
def _filter_df_by_query(df: pd.DataFrame, qq: str | None) -> pd.DataFrame:
    try:
        ...
        src = base.astype("string").fillna("").str.lower().str.replace(r"\s+", " ", regex=True).str.strip()
...
# Ensure baseline schema BEFORE anything that writes/reads optional columns
...
# Defensive: ensure CKW columns/index exist (idempotent)
try:
    _ensure_ckw_column_and_index(engine)
except Exception:
    pass

# Apply WAL PRAGMAs for local SQLite
try:
...
# Show a one-line runtime banner on the first tab
with _tabs[0]:
    _debug_where_am_i()
    st.caption(
...
with _tabs[0]:
    ...
    # Optional help expander (secrets-driven)
    if _as_bool_patch3(st.secrets.get("SHOW_BROWSE_HELP", False), False):
        md = _load_browse_help_md()
        if md:
            with st.expander("Help — Browse", expanded=False):
                st.markdown(md)

    # Optional search box (simple, safe)
    q = st.text_input("Search (CKW-first, case-insensitive)", value="", placeholder="Type to filter…")
    _df_show = _filter_df_by_query(df, q)
    ...
    view_cols = [c for c in view_cols_pref if c in _df_show.columns] or list(_df_show.columns)

    col_cfg = _apply_column_widths(_df_show[view_cols], dict(st.secrets.get("COLUMN_WIDTHS_PX_ADMIN", {})))
    ...
    st.download_button(
        ...
        data=_df_show[view_cols].to_csv(index=False).encode("utf-8"),
        ...
        if phone_norm and len(phone_norm) != 10:
            ...
            try:
                ...
                _exec_with_retry(
                    engine,
                    """
                    INSERT INTO vendors(category, service, business_name, contact_name, phone, phone_fmt, address,
                                        website, notes, keywords, created_at, updated_at, updated_by)
                    VALUES(:category, NULLIF(:service, ''), :business_name, :contact_name, :phone, :phone_fmt, :address,
                    ...
                    """,
                    {
                        ...
                        "contact_name": contact_name,
                        "phone": phone_norm,
                        "phone_fmt": _format_phone(phone_norm),
                        "address": address,
                        "website": website,
                        "notes": notes,
                        "keywords": keywords,
    ...
    with colA:
        ...
        if st.button("Add Category", key="cat_add_btn"):
            if not (new_cat or "").strip():
                ...
                try:
                    _exec_with_retry(
                        engine, "INSERT OR IGNORE INTO categories(name) VALUES(:n)", {"n": new_cat.strip()}
                    )
...
with _tabs[4]:
    ...
    # ---- CKW tools in their own expander (no nesting) -----------------------
    ...
    # ---- Export / Import (outside CKW expander to avoid nesting) -----------
    ...
    # CSV Restore UI — use container, not nested expander
    st.markdown("### CSV Restore (Append-only, ID-checked)")
    with st.container():
        st.caption(
        ...
        try:
            with engine.begin() as conn:
                ...
                for r in rows:
                    ...
                    vals["phone_fmt"] = _format_phone(vals["phone"])
                    vals["id"] = pid
                ...
                for (old_name,) in cat_rows:
                    ...
                    if new_name != old_name:
                        ...
                        conn.execute(sql_text("UPDATE vendors SET category=:new WHERE category=:old"), {"new": new_name, "old": old_name})
                ...
                for (old_name,) in svc_rows:
                    ...
                    if new_name != old_name:
                        ...
                        conn.execute(sql_text("UPDATE vendors SET service=:new WHERE service=:old"), {"new": new_name, "old": old_name})
...
with _tabs[5]:
    ...
    with engine.begin() as conn:
        ...
        created_at_nulls = (
            conn.execute(sql_text("SELECT COUNT(*) FROM vendors WHERE created_at IS NULL OR created_at=''")).scalar()
            or 0
        )
        updated_at_nulls = (
            conn.execute(sql_text("SELECT COUNT(*) FROM vendors WHERE updated_at IS NULL OR updated_at=''")).scalar()
            or 0
        )
...
# ─────────────────────────────────────────────────────────────────────────────
# Global h-scroll & exact pixel widths helpers (CSS/JS)
# ─────────────────────────────────────────────────────────────────────────────
import streamlit as _st_patch1
...
def _apply_exact_column_widths_from_secrets() -> None:
    try:
        cfg = dict(_st_patch1.secrets.get("COLUMN_WIDTHS_PX_ADMIN", {}))
        if not cfg:
            return
...
__apply_col_widths_once = st.session_state.get("__apply_col_widths_once")
if not __apply_col_widths_once:
    _apply_exact_column_widths_from_secrets()
    st.session_state["__apply_col_widths_once"] = True
# ─────────────────────────────────────────────────────────────────────────────
